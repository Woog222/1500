...
class checker:
    ...
    def check_traveltime(self):
        ...
        result_temp = result_temp.merge(self.od_matrix[['origin', 'Destination', 'Time_minute','Distance_km']], how='left')
        ...
        violate_traveltime = result_temp_drop[(result_temp_drop['ArrivalTime'] < result_temp_drop['NextArrivalTime'])&(result_temp_drop['NextSiteCode'].isna()==False)]
    ...
    def check_timewindow(self):
        ...
        corner_feasible_idx = result_check2[(result_check2['ServiceStartTime']>=result_check2['start'])|(result_check2['ServiceStartTime']<=result_check2['end'])].index
        ...
        normal_feasible_idx = result_check2[(result_check2['ServiceStartTime']>=result_check2['start'])&(result_check2['ServiceStartTime']<=result_check2['end'])].index
    ...
    def check_72hours(self):
        print("72시간 제약 확인 중..")
        violate_72hours = self.final[(self.final['group'] != -1) & (self.final['group']*360 + 72*60 < self.final['DepartureTime'])]
    ...
    def check_capa(self):
        ...
        check_idx = result_check2[(result_check2['VehicleID'] != result_check2['NextVeh'])|(result_check2['ORD_NO']=="Null")].index
        ...
        for num in range(len(check_idx) - 1):
            ...
            res = result_check2.loc[num1:num2]['cbm'].sum() <= self.vehicles[self.vehicles['VehNum']==veh]['MaxCapaCBM']
            if any(res) == False:
